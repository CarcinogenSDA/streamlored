# Resident Evil HD Remaster - Jill Normal Mode Bad Ending (any%) Speedrun Guide

## Overview

This guide covers the Jill Normal Mode Bad Ending (any%) route for Resident Evil HD Remaster on Steam. This run is played on "Out of the Box" game settings. The bad ending requires: not rescuing Chris from his cell, Barry dying earlier in the game, and the mansion not being destroyed.

**Category**: Any% Bad Ending
**Character**: Jill Valentine
**Platform**: Steam

**Key Route Characteristics for Bad Ending any%:**
- Say NO to Barry in caves (causes his death, and you pick up Barry's .44 Magnum)
- Don't rescue Chris from lab cell (avoid picking up MO Discs)
- More aggressive/optimized routing with a very direct route toward the end.
- No usage of Rocket Launcher or Samurai Edge.

**Characteristics of speedrunning Resident Evil HD Remaster on PC**
- In the game's settings, your framerate is set to "variable", and the Vsync option is set to "off". This makes the game run at a maximum of 120 Frames Per Second ("FPS"). 120FPS causes the game to run at its maximum speed.
- any% in the context of Resident Evil REmake speedruns (all platforms) means "any ending", but it is intended for the player to run Bad Ending, as it is the fastest.
- for any% runs, Rocket Launcher is banned unless you are running the New Game + category

**Characteristics unique to playing Jill Valentine**
- Jill has 960 HP, or "hit points". This is her health. If she is above 75% health, she is in "Fine" Status. If she has between 50% and 75% health, she is in "Yellow Caution" status. If she is between 25% and 50% HP, she is in "Orange Caution" status. If she has only 25% damage left, she is in "Danger" status.
- Jill has a maximum of 8 item slots. Chris has a maximum of 6.
- Jill gets a permanent Lockpick from Barry in the main hall at the beginning of the game. Chris gets a permanent Lighter in his inventory, instead.
- Jill will find a Grenade Launcher. Chris will find a Flamethrower, instead, but he doesn't get the Flamethrower for the whole game.

**About the Livesplit Autosplitter**
- Resident Evil HD Remaster speedruns on PC/Steam are timed via an Autosplitter Script (.asl file) in Livesplit. This Autosplitter Script automatically hits the splits so the player doesn't have to worry about manually hitting the button each time a split is completed.
- Carci's splits are "door splits", meaning the autosplitter will automatically split each time Carci opens a door, with the exception of a few doors (if necessary, will define later). With each new room load, a new split begins.
- Resident Evil HD Remaster speedruns on PC/Steam are timed via LRT, or "Load Removal Time". This is a timing method that uses the .asl script. The .asl script times the speedrun via "RTA" or "Real Time Attack" timing. This is different from Resident Evil HD Remaster's internal game timer, in that it uses "Real Time" instead.
- LRT pauses the timer on each door load, freezing the timer during loading screens. This is done to make timing more fair between PC setups, as each runner's computer hardware is different.

---

## Mansion 1

### Unequip Handgun

Open inventory immediately and unequip the handgun. Unequipping your handgun saves time since you'll be running past most of the enemies in the game.

### Wesker Dialogue

Mash through Wesker's dialogue in the main hall.

### Cold Feet (Emblem)

Check the double-doors again. This will initiate a cutscene where Barry asks Jill if she has "cold feet". Skip it. Now run to Barry and skip the "blood" cutscene. Do not pick up the Emblem, as this makes it more difficult to move the menu cursor quickly for most times you have to use items during Mansion 1. Now pass by the door where you would normally go through to find the zombie eating Kenneth, and try to go back to the main hall. An alternate cutscene will play where the aformentioned zombie comes out of the door.

### Main Hall Stairskate

After cutscenes, execute stairskate on main hall stairs going up then back down. Input: hold "up" on D-pad + mash run while approaching stairs. Mash past the following cutscene to get the Lockpick (Jill-Only Item).

### to Kenneth Hall

Head through the dining room into the door to the Kenneth Hallway.

### to West Stairwell

Head through the door next to Kenneth, into the West Stairwell 1. There are 5 total doors in the Kenneth Hallway, which will be defined later.

### West Stairwell Stairskate

Stairskate up the west stairwell. Open the door to the Arrow Hallway, or "Murder Hall".

### !!HALLWAY HUMPIST!!

This hallway is known colloquially by runners as the "Murder hall". It is the hallway where you pick up the Golden Arrow. There are two active zombies in this room, and one zombie that becomes a Crimson Head later. There are 5 doors. The one directly infront is unlocked, which leads to the Armor Trap. We can't go there yet. The next door is locked, and leads to West Stairwell 2. Immediately after this door is one zombie, who we will call Sleepy Santiago. He is mostly inactive whenever we pass around him, so we ignore him for now. Run past him, then pick up the Golden Arrow.

Zombie waiting to ambush off-frame near the mirror. Carci calls him "The Hallway Humpist". Round the corner, aggroing him, then bait a lunge. If he doesn't lunge, his lunge is on a delay, so you must do an alternative dodge. Let him walk to you for a couple of steps, then run around him using the alt-control movement. Reset if bitten.

### to Main Hall

Go through 2F Dining Room through the double doors to the Main Hall. Avoid the dagger. This dagger is an optional pickup later, but if you must pick it up, picking it up now will result in IRL timeloss if you must reset.

### to Graveyard

Go down the short stairs, through the door to graveyard.

### !!FUCKFACE FREDDY!!

There's two zombies in the graveyard area. The first one, at the base of the stairs, is called Fuckface Freddy. His friend in the left-hand alcove is Fat Fuck Fernando. Fuckface Freddy has many different places he can be, as his set 'rotation value' is completely random. Stairskate down the stairs until you see Fuckface Freddy. Then run past him and head toward the tomb with archer statue. On the way to the Archer Statue, stay to the right hand side of the pathway so Fat Fuck Fernando doesn't aggro. Fat Fuck Fernando will rarely aggro if you stay toward the right hand side, but sometimes he'll aggro. Examine Golden Arrow to get Arrowhead. Use the Arrowhead on the Crypt to gain access. Stairskate down the stairs. When in the crypt, pick up the Book of Curse. Stairskate back up the stairs and leave. If you didn't aggro Fat Fuck Fernando, he'll still remain at his spawn, and you'll only have to dodge Fuckface Freddy. If you see Fuckface Freddy on the sideways camera angle on the way out, try to bait a lunge and run around him. If Fuckface Freddy doesn't lunge, wait for him to meander into a wall and run past him. If you don't see Fuckface Freddy on the sideways camera angle, advance to the next camera angle and backstep into the previous camera angle to ascertain his position. If you have space to just run past, just do that. The ideal setup is if Fuckface Freddy is completely out of your path, and you can just run a straight line back out the door. But if Fat Fuck Fernando is active, you have to dodge them both. Reset the run if you get bit. Otherwise, if you are able and manage to clear both zombies in any situation, stairskate back up the stairs and leave through the door you came through, back into the Main Hall.

### to Art Room

In the main hall, go into your inventory and examine the Book of Curse. Hold Down-Right on the D-pad to rotate the book until you see the Sword Key on the back of it. Holding Down-Right on the D-Pad is the fastest way to rotate the book until you see the Sword Key. Hit "Confirm" to take the Sword Key. Mash Cancel until you regain control of Jill. Stairskate down the stairs then take the double-doors on the right-hand side of the Main Hall, into the Art Gallery with the statue of the Woman Drawing Water.

### Unlock Door

In the Art Gallery ("Art Room") with the Statue of Woman Drawing Water, unlock the Sword Key door and enter the Dog Hallway

### Dog Hallway

Pass through to the door on the opposite side of the hall into the "Squiggle Hall".

### Lockpick Door

Carci calls this hallway the "Squiggle Hall" because it's shaped like a weird squiggle with 4 turns. There are 4 doors. The first door immediately next to the dog hallway (which we just came from), leads to the outside area with two dogs outside of a fence, where you find the Herbicide. This door is locked with the Small Key (Chris) or Lockpick (Jill). The next door is the bathroom with the zombie in the bathtub. The door after that leads to the Shotgun trap. The double-doors at the end of the hall lead to the Spencer Hallway, which will be defined in the "to Spencer Hallway" split. Unlock the first door with the lockpick and enter it.

### Herbicide

This exterior area is a fenced in dead-end with a small boiler and some gardening supplies. There are two zombie dogs outside, which will not jump over the fence unless you exit and return to the room. Run to the Herbicide, pick it up, then leave the room through the same door you entered, back into the Squiggle Hallway.

### to Spencer Hallway

In the squiggle hallway go through the very last set of double-doors, which lead to the Squiggle Hallway.

### to East Stairwell

The Spencer Hallway has 4 doors. The first door leads to the East Stairwell. The second door is locked with a door that has a Spencer Emblem, which is unlocked by the Spencer Key. This door is unlocked much later in the run. The third door is locked by the Armor Key, which leads to the Glass Stain Crow Puzzle. The fourth door leads to the area that leads to the courtyard, the sheltered area where you place the Stone and Metal Object later. Run immediately through the first door to the East Stairwell.

### !!Lunge Bait (Reset)!!

The East Stairwell has 2 floors, 3 zombies, and 6 doors total. Some of these doors will be defined later since they are unimportant at this point in the speedrun. On the first floor, there is one zombie at the bottom of the stairs, next to the East Save Room. Avoid this zombie and avoid going to the East Save Room and stairskate directly up the stairs. Immediately head to Jill's subjective right, which would be the "down" direction on the camera angle at the top of the stairs. This leads toward a door adjacent to a small corridor with another door on the end. 

There is a zombie next to two of the doors on the next camera angle. The first door you see on this camera is missing a doorknob on this side of the door, so it cannot be entered. This door is a one-way door coming from the Small Library. The zombie is closest to this door, around the corner leading into the small corridor. This zombie has a high chance of lunging. Run to the corner to bait a lunge, then use Alt Controls with the left analog stick to swerve downward around him, barely out of his lunge range, then run to the door directly behind him. This door locks from one side, unlocking from the East Stairwell side of the door, to the C-Shaped Hallway. Go through the door.

### !!5% FRANKIE!!

Now we're in the C-Shaped hallway. You'll see the Wooden Mount next to the door you just entered from. DO NOT PICK IT UP. It's useless. There are 5 total doors in this room. They will all be defined in the split titled "Loose Larry". Directly to Jill's subjective right is a door at the end of the hallway. We want to go through that door, but there's one zombie in the way, which you will see immediately This zombie is called "5% Frankie" because he has a 5% chance to grab you. In order to get around him, simply run to the left hand side and hug the wall to get around him. Then you can enter the door to the Small Library (also called the Small Study).

### Lighter, Dog Whistle

Entering the Small Library (also called "Small Study"). There's two doors. The door you enter from is from the C-Shaped Hallway, the other door leads to the East Staiwell. On the other side of that door is the door leading to the second floor of the East Hallway, next to where we dodged a zombie earlier. If you remember, this door mentioned before is a one-way door. You can pass through it and not be able to come back through, so do NOT go through this door. There's a circulation desk directly in front of the door you came in from, with a Lighter on top. Pick up the Lighter. Then turn around immediately and go to the side table next to the door you came from. There is a Dog Whistle sitting on top of this table. Pick it up, then mash out of the following file screen that pops up. Now leave back through the door you came through.

### !!LOOSE LARRY!!

On the way back out from the Small Study, run past 5% Frankie and the door to the East Stairwell. There is a Wooden Mount. Do not pick up the Wooden Mount. After turning the corner, another zombie, which Carci calls "Loose Larry", whose spawn position is next to the locked double doors in the middle of the C-shaped corridor. If you take the corner loosely, hugging the outside wall, you can trigger the next camera angle at a safe angle of approach which will show where Loose Larry will be. Otherwise, you have to blind dodge him. There will be another set of locked doors to pass, and the door at the end of the hall can be opened by the Sword Key. Use the Sword Key to unlock that door and go into the Main Hall.

### Cross to 2F Dining

Because we don't trigger the Shotgun Trap (Jill Sandwich Cutscene) and we don't go to the East Save Room, Barry is not in the main hall. We do not receive acid rounds. We do not need the acid rounds anyway. Go across the walkway to the 2nd Floor Dining Hall.

### Unlock Door

In the 2F Dining Room, unlock the Sword Key Door. Optionally, you can pick up the Dagger in here, which was mentioned before. Enter it to go into West Stairwell 2.

### Lockpick Door

You've entered West Stairwell 2. Directly next to the door you just came in from, from 2F Dining Room, is a door to the West Terrace. Unlock it with your lockpick and go through it.

### !!QS Dogs (Reset)!!

You're on the West Terrace. Equip the Handgun (Slot 1). Use the Dog Whistle (Slot 6). 2 dogs will spawn at the far end of the terrace. You will hear dogs panting, then you will hear a chain dragging. After about 1 second after you hear the chain dragging, try to quickshot. If you can not chain quickshots, fire 3 shots then wait for the approaching dog, "Toby", to throw its head up. When it throws its head up, it's using a grab attack. Shoot the dog one more time when it throws its head up, so that the attack is cancelled. If the dog is far enough, it will then tackle Jill, knock her down, and try to rip out her throat. This attack on its own does no damage, but it can kill you if you don't mash buttons. You can counter this attack with a dagger. If you've done 4 shots worth of damage with the handgun to the dog, then the dagger should kill the dog. Otherwise, shoot one more time. Pick up the collar the dog drops before the second dog runs in to grab you or attack you. Open the inventory, unequip the handgun, then exit through the door you came in from. The game asks if you wish to discard the Dog Whistle since it is no longer useful. Say "Yes" to discard the Dog Whistle.

### !!GRABBY GABRIEL!!

When you re-enter the West Stairwell 2, run around the staircase towards the other side of the 2F level of the stairwell. There is a door locked with the Helmet Key, and a door which unlocks into the Murder Hall (Arrow Hall). In front of this door is a zombie called Grabby Gabriel. Grabby Gabriel sometimes has a high lunge timer, so you may not be able to bait a lunge very quickly. When you bait the lunge, run around him, and unlock the door to the Murder Hall.

### to Key Trap

The door from the West Stairwell is very close to the door to the door to the Armor Trap Hallway. As you go to the Armor Trap Hallway, the dead body rises and becomes a Crimson Head zombie. Ignore him and go through the door to the Armor Trap Hallway.

### Armor Key

Stairskate up the stairs, and proceed through the hallway until you find the Armor Key on an inset on the floor. Take it. This activates a trap, where you have 10 seconds to either put the key back into the inset or put a suitable replacement in the inset, or the trap kills you. Open the inventory, then examine the Dog Collar we picked up before. Press "confirm" to open it. A "coin" will come out of the collar. Then rotate the coin to find the same emblem on the Armor Key. Examine that, and the coin turns into the shape of a key, becoming the Fake Armor Key. Now replace the Armor Key with the Fake Armor Key. The other door at the end of the hall is locked and has a Helmet Key Emblem, which we can't open until later, so turn around and head back to main hall 2F.

### !!Murder Hall!!

We're back in the Arrow Hallway, and now that the Crimson Head has awakened, it is now called the Murder Hall. It is called Murder Hall by speedrunners because many speedrun attempts end here. It is very easy to take so much damage here that you'll get thrown into Orange Caution status, so Carci employs a possible backup strategy here.

Round the corner towards the door to West Stairwell 2. The Crimson Head will be right behind you. Sleepy Santiago, who we normally ignore, has a chance of being in your way now. If Santiago is directly in your path, don't try to bait a lunge, because the Crimson Head will hit you. Instead, go into the West Stairwell 2 door and go out to reset his position. Upon reloading the room from that door (or if you pass Sleepy Santiago) we can run towards the Hallway Humpist, and dodge him the same way we did before. Now we exit through the door that leads to Dining Room 2F.

### to Main Hall

In Dining Room 2F from Murder Hall, Return to main hall.

### to Terrace

in Main Hall 2F, turn right then stairskate up the small steps. There's a locked door which would lead to the 2F terrace, but we will never unlock that door. There's no reason to. Cut across the walkway next to the door to reach a door which you unlock with the Armor Key. This leads to the Terrace which has Forest's body.

### Grenade Launcher

Follow the path all the way to Forest's Body. A cutscene plays here because we didn't trigger Barry's appearance in the Main Hall. In this cutscene, Barry talks to Jill about Forest's demise, then gives her the Grenade Launcher that Forest was holding. Optionally, you may pick up the Dagger on the bench if you don't feel confident with your zombie dodging, but it isn't necessary. Return to the Main Hall through the only door.

### to 2F E Hall

Go down the short stairs next to the door you came out of, to return to the C-Shaped Hallway.

### to Richard

In the C-Shaped Hallway, go though to the first door on the right, and unlock it with the Armor Key. This leads to the Richard Corridor, where Richard is.

### Richard Cutscene

Upon entering this room from the C-Shaped Hallway, a cutscene plays where STARS Bravo Team member Richard Aiken has been bitten by a huge snake and is poisoned by snake venom. Skip it, then the game will send you back out into the C-Shaped Corridor.

### to Main Hall

A map screen will pop up telling you to go to the West Save Room (also known as the "Infirmary"). Mash out of it. You now have 4 minutes to save Richard by picking up some Serum from the Infirmary and bringing it back to Richard. It is faster to save Richard than it is to let him die. Richard must either be saved, or he must die in order to proceed through the door beyond the Richard Corridor, but it is faster to SAVE Richard. Saving Richard does not have any influence on the ending whatsoever, and anywhere you heard that is completely 100% wrong, because I am Carcinogen and I am correct when it comes to Resident Evil REmake. Proceed back through the door to the Main Hall.

### to 2F Dining Room

Go directly across the walkway to the 2F Dining Room.

### to W Stairwell 2

Proceed through the door to West Stairwell 2.

### !!EMESIS EDDIE!!

Upon enteringthe West Stairwell 2 from 2F Dining Room, a zombie is trying to open the door from the terrace that we fought the dogs and picked up the collar previously. Currently in this room, there's two zombies. There's Grabby Gabriel, who we don't even see this trip, and another zombie who may be on the staircase. Circle around to the staircase. When you approach the staircase, this triggers the zombie to bust in from the West Terrace. Ignore him and proceed down the stairs. The next zombie, Emesis Eddie, may either be on the stairs or at the bottom of the stairs. Ideally, we want him to be ON the stairs, because if he's on the stairs, we can stairskate past him, since he will try to puke on us and miss. Zombies puke when they're on the stairs, and while they're puking, they are easy to get around. If Emesis Eddie is at the bottom of the stairs instead, slow down, check his position, and dodge around him to get to the West Save Room (Infirmary). He may be at an awkward angle at the bottom of the stairs, so he may grab us and bite us. Proceed through the door to the West Save Room.

### Deposit HG, Knife, Sword Key

Medical Supply save room. Take the Serum. Deposit the Handgun, Knife, and Sword Key into the item box. Leave back into the West Stairwell.

### Unlock Door

We're on the first floor of the West Stairwell. Round the corner, then proceed to the end of the corridor (do not go back upstairs). A zombie is in our way, he is very tricky to dodge. Once you get past him there are two doors. One door can be opened with a lockpick. The door next to it can be opened with the Armor Key. Unlock it with the Armor Key, to proceed to the F-Shaped Hallway.

### to Plant Room

Grab battery pack from table. Run past windows to door at end.

### Death Mask 1

Put Chemical in water pump, turn handle to red. Water kills plant. 5 green herbs available. Grab Death Mask in back past herbs. 2 zombies crash through window on exit - run away.

### to Kenneth Hall

Head toward bottom of F-shaped room. Through door to Kenneth's hall.

### to Main Hall

Go to dining room. **BAD ENDING**: Skip grabbing wooden emblem here since we already have it from opening.

### to 2F E Hall

Head back to 2F east hall.

### to Richard



### Richard Serum Cutscene



### !!Sleeper!!

Through room past Richard. Zombie in corridor where you turn.

### !!Music Notes Lunge!!

In dark hall room, light candles on table. Check room for ammo. Push right shelf toward camera. Zombie appears on angle change - run around table and shoot. Grab Musical Score from alcove.

### to Richard

Leave room, back through Richard's room.

### to 2F E Hall

Go to corridor before Richard's room, find green double doors.

### to Main Hall

In knight statue room, push in order: upper right, lower left, lower right. Press table button, grab Jewelry Box. Press action on halved-heart sides for Death Mask.

---

## Mansion 1F - Piano & Emblem

### to Dining 1F

Go back to hall, unlock door at bottom of stairs with Sword Key and discard it (nothing valuable down there).

### to Kenneth Hall

Go back up and through first door on left.

### !!Dodge with Stick Only!!

Dodge zombie using only movement - no shooting.

### Gold Emblem

Piano room - push shelf to find Moonlight Sonata cover. Combine with Musical Score, play on piano. Wall opens to secret passage. Grab Gold Emblem (and Trevor's Diary), replace with Wooden Emblem.

### !!MAY BACK GRAB!!

Zombie may grab from behind when returning.

### Shield Key

Return to dining room, put Gold Emblem above fireplace. Go to grandfather clock, move large hand twice (either direction) to read 6 o'clock. Get Shield Key.

---

## Mansion 2F - Yawn & Death Masks

### to 2F E Hall

Head to 2F east hall. Go to room past Richard (grab green herbs if needed).

### to Richard

**BAD ENDING**: Richard is dead/dying - no Assault Shotgun from him. Watch out for Crimson.

### to T-Hall

Walk up steps.

### to Yawn 1

Unlock door with Shield Key, discard key. Walk toward back of room.

### Death Mask 2 !!YAWN 1!!

Yawn attacks! **BAD ENDING**: Richard doesn't come to save you - you're on your own. Keep avoiding snake while grabbing Death Mask and Shotgun ammo. Yawn goes down but isn't dead.

### NOOOOOOOOO

If bitten - you're poisoned. Return to save room for serum if needed.

### !!ZOMBIE HERE NOW!!

Head back to first floor. Through double doors. In bathroom hallway, 4 zombies crash through windows.

### !!QUICK ROTATION!!

Quick camera rotation for dodge through L-shaped hallway.

### Death Mask 3

Zombie around corner. Kill and enter door to left that you can unlock.

### to 2F E Stairs

Head to 2F east stairs.

### !!1% BACKGRAB!!

Very low chance of back grab.

### Flame Rounds

Grab flame rounds from Barry's items in save room.

### !!TIGHT DODGE!!

Tight zombie dodge required.

### to Crow Puzzle

U-shaped hallway with paintings. Crows here won't hurt you if you do puzzle right.

### Death Mask 4

Change first painting to orange. Run to other side of U. Flick lights on first two paintings (purple and green). At end, woman (Lisa Trevor) wearing orange/purple/green. Press switch. Wall raises - go outside, grab last Death Mask, unlock gate.

### !!ELDER CRIMSON!!

Watch for Crimson Head in area.

### to Spencer Hall

Head down to grave area with coffin. Fastest way through gate next to Death Mask. Put all 4 Masks on wall (match them up). Shoot the Crimson with Shotgun (~5 rounds). Grab Stone and Metal Object and Shotgun shells.

### !!SLEEPER!!

Sleeping zombie in Spencer hallway.

### Insert Object

Head to hall outside Crow Gallery. Go straight through metal door to outside. Place Stone and Metal Object. Dog jumps out - easy to avoid.

---

## Courtyard & Forest

### to Woods

Tool shed has two doors. Take the one down small steps. Grab F.A. Spray and battery pack by steps. Watch cutscene.

### Cocks

Head down path. Make red weathervane point West.

### Crows

Blue weathervane further down points North. Go through gate. Avoid crows.

### Winding Path

Graveyard with crows has two graves. Put Wind Crest on right grave. Three more crests appear - examine all backs. Put all on left grave to get Magnum!

### Crank, Box !!LISA!!

Continue path to cabin (Trevor's Shack). Inside: typewriter, item box, Square Crank (off ledge by item box), Courtyard map upstairs. Dump Magnum, keep weapon and Square Crank. Lisa attacks and knocks you out - she's invincible, run past and escape!

### !!ZOMBIE!!

Outside cabin, grab green herbs. Go back to tool shed - zombie by tree in walkway, stands still until fired at.

### Nothing

Empty transition.

### Nothing

Empty transition.

### to Courtyard

Through other door in shed. Quick cutscene. 3 dogs attack - kill them. Blue and green herbs here.

### !!3 DOGS!!

Kill the three dogs.

### Turn Crank

Up small stairs through gate. Use Square Crank to lower water. Down ladder, run across, up other ladder.

### Crows are Harmless

Take elevator down. Garden area with crows - just run, grab Green Herb. Through gate on other side of waterfall.

### to Guardhouse

Red herb near. Watch for snakes - if bitten, poisoned. Through door at end. Mansion complete!

---

## Guardhouse

### to Bar

Run straight forward, grab all 4 blue herbs and heal if poisoned. Turn around, head left to door on right - save room with battery pack. Drop unneeded items including crank, but keep lighter.

### Red Book !!SPIDER!!

Exit save room, turn right to double doors. Spider above you! Another in room but won't attack if left alone. Green herbs available. Three colored lanterns: green, red, orange. Light each and note color + number of ticks on eyeball. Green lantern downstairs with Red Book (Bondage With Blood) and First Aid Kit. Match ticks to pool ball numbers.

### !!UROTSUKIDOUJI!!

Tentacle plant enemy - Plant 42's tentacle can grab you.

### to Room 002

Push box by double doors toward camera, into small corridor over hole. Climb boxes to get down. Enter double doors, keep going past first door to room 002. Jill hears voices - cutscene.

### Barry Cutscene

Barry cutscene in Room 002.

### 001 Key

Door next to entrance leads to small bathroom. Grab 001 Room Key from shelf.

### Exit 001

Zombie outside room. Go back toward save room, door across from it. Discard room key.

### to Tentacle Hall

Past hanged dude, search around. Self-defense gun (1 bullet, 1-shot kill). Grab items on chair/table.

### !!UROTSUKIDOJI!!

Back to bathroom by entrance door. Zombie on floor - waste it.

### 001

Get Control Room Key from bathtub after draining.

### C. Room Key

If loud crash heard, hanged guy fell and is now zombie - ignore him.

### 001

Head back to room 002.

### !!UROTSUKIDOJI!!

Zombie still here if not killed earlier.

### to 002

Two shelves in back of room. Push left shelf all the way back. Push right shelf right until ladder visible.

### Shelf Glitch

Perform shelf glitch for faster access.

### Box Puzzle

Go down ladder (no enemies). Push boxes into water in corridors. If mess up, go up and come back. Grab green herb.

### !!SHARKS!!

Enter flooded double doors to Aqua Ring. Follow wall toward camera until cutscene. Run like hell up stairs!

### 3, 1, or 2?

Use Control Room Key, discard. Climb down ladder. Grab First Aid Kit to left. Map above whiteboard. Check whiteboard for valve info.

### to Neptune

Un-flood Aqua Ring: Hit switch to right, then switch behind ladder. Shutter stuck. Turn valve mentioned on whiteboard. Hit first switch again, then shutter switch, then first switch to drain. Exit right side, down stairs through unlocked door.

### !!NEPTUNE SKIP!!

Gate at far side - don't go yet. Head other way through other gate. Grab Grenade Shells. Sharks just flop around. Run to Neptune (big shark). Climb platform, try to grab Gallery Key - it drops when Neptune wakes. Push green control panel into water, pull lever to electrocute shark.

### to Ladder Room

Grab Gallery Key. Back to previous room through other gate. Magnum rounds on box - grab but don't use yet. Climb ladder through door to Aqua Ring B1.

### to 002

Climb back to Room 002, exit Aqua Ring. Don't grab Residence map yet (bees will poison you).

### !!ZOMBIE!!

Make pit stop at Save Room to unload. Bring Gallery Key.

### Unlock Gallery

Head to gallery door by room 002.

### Insecticide

Grab handgun clip dodging bees. Follow bees to door past room 003. Take Insecticide Spray. Residence Map to right of 002 - hole nearby, use Insecticide in it.

### Use Insecticide

Bees gone when you return. Grab Red Herb down hall, then Room 003 Key further down.

### 003 Key

Back to save room. Need 2 inventory slots, bring Red Book (Bondage With Blood).

### She naked

Room 003 - bookshelf in back. Grab only book you can, replace with Red Book. Rearrange: woman's feet at bottom left, hands at top right. Shelf slides revealing secret door.

### Plant 42

First check other door in gallery (not doubles). Match tick marks with colors/pool ball numbers. Inside: 4 empty bottles (need 3), UMB no.3, Yellow-6, sink.

**V-Jolt Recipe:**
1. UMB no.3 + Water = NP-004
2. NP-004 + Yellow-6 = UMB no.10
3. Yellow-6 + Water = UMB no.7
4. UMB no.7 + UMB no.10 = VP-017
5. VP-017 + UMB no.3 = V-Jolt

Go back to Aqua Ring upper level (shark door). Use V-Jolt on Plant 42 roots. Return to room 003 secret door. **BAD ENDING**: Barry still saves you here - this doesn't affect bad ending.

### To Hallway

Grab Helmet Key from fireplace when plant dies. Exit room.

### Wesker Cutscene

Exit Gallery for Wesker cutscene.

### Leave Guardhouse

Continue to save room, drop supplies, bring Shotgun.

### Skip Cutscene

Skip cutscene.

### !!DOGS!!

Two dogs near waterfall ate the birds - kill them.

### !!SNAKES!! (Reset if Poison)

Head back to mansion. Snakes will likely poison you - Blue Herb at mansion. Reset if poisoned and no cure.

### !!DOGS!!

More dogs on path.

### to Mansion

Return to mansion. Grab new items in toolshed. Remove Stone & Metal Object if going for 100% items (re-locks Residence door).

---

## Mansion Return - Hunters

### Passing Through

Cutscene plays - Hunters now spawn in mansion. One attacks immediately - kill it.

### Enter Hunter

Go back to main hall via crow gallery to avoid more Hunters. Blue gem on left side of dining room table (opposite grandfather clock).

### !!WILL HE JUMP BACK?!!

Head upstairs through dining room balcony toward save room. Two hunters on Normal/Hard.

### unlock door

Before stairs to save room, go into small corridor. Eagle statue on wall moves. Turn off lights.

### Doom Book 1

Grab ammo and Dagger on shelf by yellow gem. Push dressers under animal heads with gems. Hear "chink" sound so eagle stops. Hug wall to dresser, climb up, grab gem. Repeat for other gem (red and yellow).

### !!RANDOM SPIDER ACID!!

Head to F-shaped hall (by broken Shotgun room). Hunter jumps through each window - murder them.

### !!2 ZOMBIES!!

Enter door on shorter hall of F. Place blue and yellow gems in tiger statue. Get Shotgun shells and first MO Disk.

### !!PROBABLY SLEEPING!!

Head to main hall, unlock door you haven't been to on main floor. Ink ribbons in drawer (Easy/Normal).

### Elevator to 2F

Find other door. Single zombie roaming - easy to avoid.

### !!TIGHT LINE ZOMBIE DODGE!!

Grab green herb, Dagger, jewelry box. Blue herb planter here. Combine red gem with jewelry box to open.

### Battery

Jewelry box puzzle: Third piece → bottom right. Top piece → upper left (rotate if needed). Fourth piece → between others. Last piece → right side open.

### to Murder Hall

Get Brooch. Examine it → becomes Emblem Key (one door only).

### Reverse Murder Hall

Head through crow gallery. Unlock door with Emblem Key - the Study. Turn on desk lamp, grab Metal Object. Leave Metal Object in item box (East save room).

### to Yawn 2

Grenade Launcher recommended. Go to dining room balcony, through first door this time. Back to Armor Key room, all the way down to door. Unlock with Helmet Key.

### Doom Book 2 !!YAWN 2!!

Green herb (2 Easy, none Hard). Keep going - Yawn attacks! Run and climb down ladder. Find row of books to go behind. Yawn follows in one end, come out other. Blast its tail. Go up ladder - Yawn can't hit you. Hit its head. When dead, pick up book it knocks off. Examine for Wolf Medal.

### to Murder Hall

Go to save room (canteen room). Put medal away.

### to W Stairwell 2

Hunter near save room door - blast it. Another upstairs on Normal/Hard.

### unlock door

Head upstairs, turn left, walk to corner, enter door. Grab herbs if needed (keep 2 spaces open).

### Red Jewel, Grenade Rounds

Through next door. Push statue in opening to left - walls close but statue stops them.

### to Dining Room

Go where statue was, into other passage. Switch at end to reset walls. Push it, run back, push statue left to indentation.

### !!MANIPULATE TURN AGGRO!!

Where it opened: battery pack (Easy), Dagger (Normal) on cabinet.

### to Dressing Room

Go down. Grab book, examine for Eagle Medal. Check grave and head down.

### Jewelry Box !!ZOMBIE!!

**Basement**: Three spiders (Normal/Hard, 2 Easy). Kill first, at T kill second on left. Grab Mansion B1 map. Continue around corner for Shotgun shells.

### Main Hall

Through door. Kill two zombies. Items: Red Herb, Blue Herb, Dagger. Flip switch at end of shallow water hallway to activate elevator. Grab Handgun Magazine by door.

### Graveyard

Exit to kitchen. Grab Dagger, kill crawling zombie (or Crimson if visited before).

### Crows

Find elevator. Corpse on floor. Call elevator up, head in and down.

### !!RUN STRAIGHT FORWARD!!

Two zombies below (one each side). Come toward camera first. Herbs available.

### Passing Through

Go to other side of elevator, first room by statue. Inside: battery, battery pack, magnum rounds, acid shells.

### to Courtyard

If out of space, go through other door to save room. Take Battery and Square Crank. Deposit medals and MO disk.

### !!DOGS!!

Travel back to waterfall. Watch for dogs and snakes. Blue Herb planter near waterfall if poisoned.

### !!SNAKES!! (Reset if Poison)

Head down, find elevator, place Battery and use it.

### Use Battery !!DOGS!!

Familiar area. Use Square Crank to fill pool.

### !!DOGS!!

Use elevator to go down.

### Fill Water

Waterfall gone - head down into passage and ladder into depths below.

### !!DOGS!!

Watch for dogs on return path.

### !!DOGS!!

More dogs.

---

## Caves

### to Enrico

Down the ladder, door next to you - don't go in yet. Keep going forward. Put away Square Crank in item box (won't need again). Take Shotgun or Grenade Launcher.

### Nothing

Empty transition.

### Nothing Again

Go back to door by ladder. Item on Easy. Go into door near handgun clip.

### Hex Crank !!HUNTER!!

Follow passage to find Enrico Marini. Watch cutscene, get Hex Crank from his body. Hunter appears when leaving - run or kill, not coming back.

### Nothing

Empty transition.

### !!HUNTERS!!

After elevator room, another hunter (2 on Normal/Hard). Go back to item box. Flame Rounds work best on upcoming spider boss.

### Jewelry Box Puzzle, use Hex Crank

Use Hex Crank on wall (you'll see where). Head through door.

### Boulder

Back up toward boulder. When you move forward, it rolls - run until you reach doorway so it passes. Shotgun shells where boulder was.

### !!BLACK TIGER!!

Through doors boulder opened. Fight and kill Black Tiger (giant spider). Smaller spiders come out after - exit and re-enter to clear them.

### Blue Herb if needed

Grab blue herb if poisoned.

### Turn Crank 3x

Find knife, equip it, use on spider web door. Another boulder - use crank THREE times on wall. When boulder starts, run to safety.

### Statue Puzzle

First aid kit where boulder was. Enter door. Push statue in front of maroon wall part. Use Hex Crank on wall - statue pushed out.

### FAS if needed

Push statue onto round platform (turns 90°). Push off, back on to face proper way. Move off, push into indentation by other statue. Get Cylinder.

### to Black Tiger

Head back to elevator.

### !!BLACK TIGER!!

Watch for remaining spiders.

### to Save Room

Find control panel, get Shaft. Combine with Cylinder. Put combined item back in panel.

### Don't Item Box

Skip item box for now.

### !!HUNTERS!!

Watch for hunters on path.

### 4231

Press 4231. Start elevator, watch scene. Item box: put away Hex Crank and big weapon (won't need).

### to Lisa Donut

Find only door, enter. Lisa appears in direction you head first - when choice comes, start left but then go right. When camera changes, note switch in wall (ignore for now).

### !!LISA!!

Keep going until door - go through. Climb box pile in corner for items.

### Push Box

Go back down, find box and push onto metal.

### !!LISA!!

If Lisa coming, go other way.

### Broken Flamethrower

Hit switch, go back to room before Lisa. Watch cutscene. Get Metal Object from item box, go down ladder (ammo on Easy). Push box into crusher. After crushing, jump down, get broken flamethrower.

### !!LISA 2!!

Go back to Lisa's place, use the switch. Find stairs, go down. Find door with hooks. Place flamethrower to unlock door and leave.

### Stone Ring

Watch for snakes. After water, pick up jewelry box. Examine to find frame for Metal Object. Dagger on bed (Easy). Climb ladder.

### No Item Box

Combine frame with Metal Object. Time to get first Stone and Metal Object back.

### !!ZOMBIE!!

Zombie that appeared after first Lisa - Crimson if killed, normal if not. Kill or leave.

### !!2 ZOMBIES!!

Graveyard (Magnum area) - crows replaced by two zombies (Very Easy). Dodge or kill.

### Passing Through

Transition area.

### Passing Through Again

Another transition.

### GET THE S&M OBJ

At Mansion, grab first Stone and Metal Object (re-seals forest/residence path).

### Unlock Spencer Door !!HUNTER!!

Go to Mansion main room. Double doors under stairs - place both Stone and Metal Objects.

### Metal Object

Item box down a ways. Bring herbs plus Wolf and Eagle Medals. Save now - typewriter at top of stairs.

### !!HUNTER!!

Hunter encounter in mansion.

### Crow Room

Navigate through crow room area.

### Graveyard

Return to graveyard.

### Place Objects

Go all the way down ladder.

### WEEEEEEE

Slide down to Lisa arena.

### SAY NO TO BARRY !!LISA 3!!

**BAD ENDING CRITICAL**: Don't give Barry his gun back! If you don't return it, he dies but you get the most powerful gun in the game. Note: You will NOT get "Trust Him" and "I'm a Member of S.T.A.R.S." achievements.

If you give gun back, Barry helps push off four pillars. Without Barry, you fight Lisa alone - push off all four pillars yourself to make her fall.

After done, metal gate opens. Examine coffin for file (only item). Head outside.

### Place Medals

Place Wolf and Eagle Medals on sides of pool. Head down elevator to Lab.

---

## Lab B3

### Nothing

Down elevator into Laboratory. Walk around to find door "won't open unless first class emergency." Continue past, down ladder to small room with typewriter and item box. Grab ink ribbons, save. Equip handgun. Get first MO disk.

### Nothing

Through door into Laboratory. Two zombies - one around corner, one up stairs.

### !!ZOMBIE!!

Kill zombies, head left. Pick up second MO disk.

### !!SHOOT ZOMBIE!!

Down stairs to B3F. Zombie to left, one straight ahead. Kill and decapitate/incinerate both - prevents Crimsons later.

### JOHN ADA CELL

Enter door right of long stairs. First Aid Kit and double green herb. Through immediate left door - blue-tinted room. X-Ray of Clark on boxes, X-Ray of Gail on left. Letter of Researcher on desk.

Place Clark X-ray left, Gail right. Flip switch - red light shows organs: Colon, Esophagus, Liver, Lungs = "CELL"

Go to double doors upper-northeast. V-ACT Note, battery pack, First Aid Kit. Computer: login JOHN, password ADA. Unlock B3F door. Password for Visual Data Room = CELL.

**BAD ENDING**: Don't unlock Chris's cell - skip that option entirely.

### to Stairs

Head to stairs. Hallway end has electronically locked door with three levers (needs 3 MO Disks to open Chris's cell - skip for bad ending).

### to Media Room

Enter newly unlocked room on right of hallway. Naked zombie (Normal/Hard) - kill it.

### P. Room Key

Pick up Slide Filter and FAX file. Ink ribbon in sink (Easy). MO Disk Reader here (skip if not saving Chris). Take Slide Filter upstairs to Visual Data Room.

### to Downstairs

MO Disk on desk past Visual Data Room.

### to Power Area

Visual Data Room: First Aid Spray on table, MO Disk on left shelves. Key code panel on left needs 4 digits.

Use Slide Filter on projector. Last slide shows "8462". Enter code - door opens to Editing Room. Get Laboratory Key, watch Kenneth's film if desired.

### !!NAKED ZOMBIE!!

Downstairs, unlock two doors southeast of central square. Discard key.

### !!CHIMERA!!

Left door: Chimeras! Push shelf forward, enter ventilation shaft. Immediately go left vent before Chimeras get you. End up in morgue with MO Disk Reader (skip for bad ending).

### !!NAKED ZOMBIE!!

Push shelf forward, back to central area. Other door you unlocked - T-shaped hallway. Kill naked zombies (Normal/Hard), especially right one.

---

## Lab B2

### to W. Hall

North side: large elevator (needs power). South side: large door to boiler room (Chimeras everywhere). Middle: small door to Safe Room.

### to Nitro Room

Safe Room: ink ribbon, First Aid Spray, grenade rounds. **SAVE HERE** - Nitro task is very tricky, can result in explosive death!

### !!BAIT LUNGE, GET NITRO!! (Can die here)

Equip Grenade Launcher/Shotgun. Enter boiler room, immediately right - kill Chimera. Take empty Fuel Supply Capsule.

Back to room with refueling device (northern side of lab). Fill capsule with Nitro compound - EXPLOSIVE if disturbed! Just press action, don't go to inventory.

**NITRO EXPLOSION CHANCES:**
- Powerful weapon fire: 25%
- Zombie grab: 30%
- Chimera grab: 20%
- Running: 1-3 sec (Hard), 2-4 sec (Normal), 4-6 sec (Easy)

### Walk 3, run 2

Walk 3 steps, run 2 - careful Nitro pattern.

### Run 3, walk 3, Run 4, walk 3, Run 3

Continue precise movement with Nitro.

### Run 1, walk 3, Run 3, walk stairs, Run 3 !!ZOMBIE!!

Continue pattern. Zombie at end - don't get grabbed!

### !!CHIMERA!!

Chimera in boiler room area.

###

Place Capsule in machine in boiler room. Deep sigh of relief!

### Turn on Power

Evade Chimeras, enter south door. Third MO Disk Reader here (skip for bad ending). Use heavy weapon on Chimeras first.

###

Keep heading south to door.

###

Room with Triggering System. Go to computer, activate power to elevator.

### to Elevator

Ready for final boss! Back north to T-corridor. Safe Room on right - SAVE. Equip health items and best weapon with ammo (Magnum, Grenade Launcher).

### Elevator to B4

Head to large elevator, activate switch. **BAD ENDING**: Barry is dead so no cutscene - head down alone.

---

## Lab B4 - Tyrant

###

Down short hallway, through door. Cutscene - mysterious traitor shows Tyrant.

### !!1 SHOT THE TYRANT!!

Tyrant breaks free! **BAD ENDING**: You have Barry's Magnum from not returning his gun - shoot Tyrant and it dies in ONE HIT!

If you gave Barry his gun back (not bad ending), keep distance and fire acid grenades until it falls.

###

After Tyrant falls: **BAD ENDING**: Barry is dead, so search traitor's body for Master Key (you need this!). Unlock door via computer in northeast corner.

### Elevator to B3

Back out of room, up elevator. Self-destruct activated.

###

Skip MO Disk door (not saving Chris for bad ending).

###

Head upstairs to B2F, through door to Safe Room.

### Stairs

**SAVE**. Make sure at least one space open!

### Ladder

Up ladder, through large door on immediate left. First Aid Kits and ammo if needed.

### Open Final Hallway

Run through passageway. Brad Vickers on radio asking for sign of survivors.

### Fuse

Run to end of passageway, pick up Fuse. Place in open slot next to elevator to activate it.

### Final Elevator

Take elevator up to helipad.

### Boring End

**BAD ENDING**: Head up elevator alone. Pick up Signal Rockets, use them. Brad sees flare and picks you up.

Jill escapes ALONE:
- No Barry (died at Lisa 3 because you didn't return gun)
- No Chris (didn't open his cell)
- No final Tyrant battle on helipad
- No mansion explosion
- The "boring" ending where Jill just flies away

Congratulations, you got the Bad Ending!

---

## Notes

**Critical Bad Ending Requirements:**
1. Don't save Richard (skip serum - he dies to Yawn)
2. Say NO to Barry in caves (causes his death at Lisa 3)
3. Don't open Chris's cell in lab (JOHN/ADA terminal)
4. The mansion doesn't explode

**Speedrun Techniques:**
- Stairskates: Hold run + aim while approaching stairs
- Door skips: Frame-perfect inputs
- Shoulder dodge: Lure lunge, rush past during recovery
- Reset points: Snake poison, bad dog RNG

---

## Resources

- Speedrun.com RE1 Remake leaderboards
- Carcinogen's Twitch streams for advanced strats
